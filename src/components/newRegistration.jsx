  ...
  const handleSubmit = async (e) => {
    ...
    if (step === 4 && submitCount === 1) {
      if (Object.values(formData).every((field) => field.trim() !== "")) {
        ...
        if (medicalReportFile.length > 0) {
          medicalReportFile.forEach((file) => {
            data.append("medicalReports", file); // ✅ No change needed if backend uses upload.array()
          });
        }
        ...
        try {
          ...
          if (response.ok) {
            ...
            setSubmitCount(0); // Ensure this is correctly defined
          } else {
